...
#  angry, fear, surprised, whispering, advertisement, dramatic, narration, crying, happy, sarcastic, clear throat, sigh, shush, cough, groan, sniff, gasp, chuckle, laugh.

conditional_decoder_path = download_model("conditional_decoder", dtype=dtype)
speech_encoder_path = download_model("speech_encoder", dtype=dtype)
embed_tokens_path = download_model("embed_tokens", dtype=dtype)
language_model_path = download_model("language_model", dtype=dtype)
...
# text = f"Oh, that's hilarious! [{emote}] Um anyway, how are you doing today?"
# text = "God, that's terrible! [angry] Why are you doing this?"
text = "Let me tell you a secret! [whispering] There is no one in the office today!"
# text = "Oh, that's hilarious! [laugh] Um anyway, how are you doing today?"
target_voice_path = "/home/jvk/workspace/playground/playground/huggingface/tts/source/B_MALE1.wav"
output_file_name = "output_laugh.wav"
print(f"Generating speech for emote: {emote}")
print(f"Input text: {text}")

max_new_tokens = 256 # 512
# max_new_tokens = 1024 # Reduce for faster inference during testing
